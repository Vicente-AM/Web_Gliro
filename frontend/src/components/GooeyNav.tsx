...
export const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        ...
        particle.style.setProperty('--color', `var(--color-${p.color}, #1ff080)`);
  ...
  return (
    <>
      ...
      <div className={`gooey-nav-container relative ${className}`} ref={containerRef}>
            ...
            {items.map((item, index) => (
              <li
                ...
                className={`rounded-full relative cursor-pointer transition-all duration-300 ease ${
                  activeIndex === index ? 'active font-bold text-brand-tertiary' : 'text-brand-secondary hover:text-brand-tertiary'
                }`}
              >
                <a
                  href={item.href}
                  ...
                  className="outline-none py-1.5 px-3.5 inline-block text-xs font-semibold uppercase tracking-wider transition-colors"
                >
                  {item.label}
                </a>
              </li>
